...
void PathPlan::odomCallback(const nav_msgs::OdometryConstPtr& odomMsg)
{
  ...
  if (abs(target_x_ + 0.5 - pos_x_) < 0.3 && abs(target_y_ + 0.5 - pos_y_) < 0.3){
...
void PathPlan::path_plan_alg()
{
  ...
  // sif(!path_map_initialized_) initializePathMap();
  initializePathMap();
  ...
  //each cell of PQ is (distnce,(x,y)) (enhancement)
  std::priority_queue<pair<int,coord>, vector<pair<int,coord>>, CustomCompare> reached_pq;
  reached_pq.push(pair<int,coord>(0,coord(GOAL_X, GOAL_Y)));
  ...
  while(nVisited < nCells){
    ...
    //get top node from pq (enhancement)
    // ROS_INFO("PQ length = %li", reached_pq.size());
    node = reached_pq.top().second;
    ROS_INFO("priority = %i", reached_pq.top().first);
    ROS_INFO("now processing (%i,%i)", node.first, node.second);
    if(path_map_(node.first,node.second)< min_dist) {
      min_dist = path_map_(node.first,node.second);
    }
    reached_pq.pop();
	...
	if(!hasWall(node.first, node.second, direction)){
      ...
      //add node to pq, with new min dist (enhancement)
      //normal implementation 
      // reached_pq.push(pair<int,coord>(min(path_map_(x_queue, y_queue), min_dist+1),coord(x_queue, y_queue)));

      // astar implementation with manhatten distance heuristics
      int man_dist = manhattenDist(coord(x_queue,y_queue), coord((int)floor(pos_x_), (int)floor(pos_y_)));
      reached_pq.push(pair<int,coord>(min(path_map_(x_queue, y_queue), min_dist+1) + man_dist,coord(x_queue, y_queue)));
      ROS_INFO("adding 1 item to pq");
	  }
    ...
    //terminate loop once cost of current cell is found (enhancement)
    if(node.first == (int)floor(pos_x_) && node.second == (int)floor(pos_y_)){
      ROS_INFO("terminating search after %i searches", nVisited);
      break;
    }
...
void PathPlan::setNextDestCell()
{
  ...
  // ROS_INFO("north: %f", neighbor_value_(NORTH));
  // ROS_INFO("east: %f", neighbor_value_(EAST));
  // ROS_INFO("south: %f", neighbor_value_(SOUTH));
  // ROS_INFO("west: %f", neighbor_value_(WEST));
